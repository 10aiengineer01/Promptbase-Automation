...
class CreatePromptIdea(BaseTool):
    ...
    def run(self):
        try:
            ...
            self._shared_state.set("TEXTPROMPTIDEA", newpromptidea)
            return "The prompt idea was successfully created. This is the new promptidea: "+newpromptidea
    ...
    def generate_new_prompt_idea(self, previous_ideas: str) -> str:
        ...
        response = client.chat.completions.create(
            model="gpt-4o",
            messages=[
                {"role": "system", "content": "Act as a expert in creating new unqiue prompt ideas. The user provides you with 'previous_promptideas' these are ideas already used. Your job is to create a new unique one and only output the idea noting else. Example Outputs: A prompt that assists with packing to make preparation for a trip a lot easier, A prompt that creates personalised fitnes and wellness tasks, etc."},
    ...
    def get_previous_prompt_ideas(self) -> str:
        ...
        try:
            results = notion.databases.query(database_id=os.getenv("TEXT_PROMPT_DATABASE_ID")).get('results', [])
            prompt_ideas = []
